...
def app_base_dir() -> str:
    """
    Σωστός φάκελος εφαρμογής για Python, PyInstaller --onedir και Windows EXE.
    Έτσι το app βρίσκει πάντα δίπλα του το lexiko.db και τον φάκελο audio.
    """
...
DB_PATH = os.path.join(APP_DIR, "lexiko.db")
...
def make_flag_pixmap(lang_code: str, width: int = 30, height: int = 20) -> QPixmap:
    """Draw small reliable flag icons so Windows does not depend on emoji fonts."""
    ...
    if code == "el":  # Greece
        ...
        stripe_h = height / 9
        for i in range(9):
            rect(0, i * stripe_h, width, stripe_h + 1, blue if i % 2 == 0 else "#ffffff")
        canton = stripe_h * 5
        rect(0, 0, canton, canton, blue)
        rect(canton * 0.4, 0, canton * 0.2, canton, "#ffffff")
        rect(0, canton * 0.4, canton, canton * 0.2, "#ffffff")

    elif code == "en":  # UK, simplified
        rect(0, 0, width, height, "#24468e")
        pen = QPen(QColor("#ffffff"), 5)
        painter.setPen(pen)
        painter.drawLine(0, 0, width, height)
        painter.drawLine(width, 0, 0, height)
        pen = QPen(QColor("#cf142b"), 2)
        painter.setPen(pen)
        painter.drawLine(0, 0, width, height)
        painter.drawLine(width, 0, 0, height)
        rect(width * 0.42, 0, width * 0.16, height, "#ffffff")
        rect(0, height * 0.38, width, height * 0.24, "#ffffff")
        rect(width * 0.46, 0, width * 0.08, height, "#cf142b")
        rect(0, height * 0.43, width, height * 0.14, "#cf142b")

    elif code == "de":  # Germany
        rect(0, 0, width, height / 3, "#000000")
        rect(0, height / 3, width, height / 3, "#dd0000")
        rect(0, 2 * height / 3, width, height / 3, "#ffce00")

    elif code == "ru":  # Russia
        rect(0, 0, width, height / 3, "#ffffff")
        rect(0, height / 3, width, height / 3, "#0039a6")
        rect(0, 2 * height / 3, width, height / 3, "#d52b1e")

    elif code == "ar":  # Saudi Arabia, simplified
        rect(0, 0, width, height, "#006c35")
        painter.setPen(QPen(QColor("#ffffff"), 1))
        font = QFont("Arial", 7, QFont.Bold)
        painter.setFont(font)
        painter.drawText(0, 1, width, height - 4, Qt.AlignCenter, "SA")
        painter.drawLine(width * 0.25, height * 0.78, width * 0.75, height * 0.78)

    elif code == "es":  # Spain
        rect(0, 0, width, height * 0.25, "#aa151b")
        rect(0, height * 0.25, width, height * 0.50, "#f1bf00")
        rect(0, height * 0.75, width, height * 0.25, "#aa151b")
        rect(width * 0.22, height * 0.42, 3, 5, "#aa151b")

    elif code == "tr":  # Turkey, simplified
        rect(0, 0, width, height, "#e30a17")
        painter.setBrush(QColor("#ffffff"))
        painter.setPen(Qt.NoPen)
        painter.drawEllipse(int(width * 0.28), int(height * 0.25), int(height * 0.50), int(height * 0.50))
        painter.setBrush(QColor("#e30a17"))
        painter.drawEllipse(int(width * 0.36), int(height * 0.25), int(height * 0.50), int(height * 0.50))
        painter.setBrush(QColor("#ffffff"))
        painter.drawEllipse(int(width * 0.62), int(height * 0.43), 3, 3)

    else:
        rect(0, 0, width, height, "#e8f0fe")
        painter.setPen(QColor("#1e3a8a"))
        painter.drawText(0, 0, width, height, Qt.AlignCenter, code.upper())
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # Τα πεδία μπαίνουν παρακάτω σε tabs για καλύτερο responsiveness.
        ...
        translations_tab = QWidget()
        translations_form = QFormLayout(translations_tab)
        translations_form.setLabelAlignment(Qt.AlignRight | Qt.AlignVCenter)
        translations_form.setHorizontalSpacing(14)
        translations_form.setVerticalSpacing(10)
        translations_form.addRow("Key (μοναδικό):", self.key_input)
        translations_form.addRow("Ελληνικά:", self._lang_row("🇬🇷", self.el_input, "el"))
        translations_form.addRow("Αγγλικά:", self._lang_row("🇬🇧", self.en_input, "en"))
        translations_form.addRow("Γερμανικά:", self._lang_row("🇩🇪", self.de_input, "de"))
        translations_form.addRow("Ρώσικα:", self._lang_row("🇷🇺", self.ru_input, "ru"))
        translations_form.addRow("Αραβικά:", self._lang_row("🇸🇦", self.ar_input, "ar"))
        translations_form.addRow("Ισπανικά:", self._lang_row("🇪🇸", self.es_input, "es"))
        translations_form.addRow("Τούρκικα:", self._lang_row("🇹🇷", self.tr_input, "tr"))
        ...
        audio_form.addRow("Audio EL:", self.audio_el_input)
        audio_form.addRow("Audio EN:", self.audio_en_input)
        audio_form.addRow("Audio DE:", self.audio_de_input)
        audio_form.addRow("Audio RU:", self.audio_ru_input)
        audio_form.addRow("Audio AR:", self.audio_ar_input)
        audio_form.addRow("Audio ES:", self.audio_es_input)
        audio_form.addRow("Audio TR:", self.audio_tr_input)

        meta_tab = QWidget()
        meta_form = QFormLayout(meta_tab)
        meta_form.setLabelAlignment(Qt.AlignRight | Qt.AlignVCenter)
        meta_form.setHorizontalSpacing(14)
        meta_form.setVerticalSpacing(10)
        meta_form.addRow("Κατηγορία:", self.category_input)
        meta_form.addRow("Tags:", self.tags_input)
        meta_form.addRow("Notes:", self.notes_input)

        tabs = QTabWidget()
        tabs.addTab(translations_tab, "Μεταφράσεις")
        tabs.addTab(audio_tab, "Ήχοι")
        tabs.addTab(meta_tab, "Στοιχεία")
    ...
    def _audio_path(self, filename: str) -> Optional[str]:
        """
        Βρίσκει mp3 με ασφάλεια σε Windows/Mac:
        - δουλεύει με PyInstaller --onedir
        - διορθώνει ελληνικά unicode NFC/NFD
        - κάνει case-insensitive αναζήτηση
        - αν λείπει .mp3, το δοκιμάζει αυτόματα
        """
        ...
        candidates = []
        if os.path.isabs(filename_mp3):
            candidates.append(filename_mp3)
        else:
            candidates.append(os.path.join(AUDIO_DIR, filename_mp3))
            candidates.append(os.path.join(APP_DIR, filename_mp3))

        for candidate in candidates:
            candidate = unicodedata.normalize("NFC", os.path.abspath(candidate))
            if os.path.exists(candidate):
                return candidate

        wanted = unicodedata.normalize("NFC", os.path.basename(filename_mp3)).lower()
